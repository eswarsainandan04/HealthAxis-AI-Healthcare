...
const Search = () => {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        setLoading(true)
        const response = await axios.get("https://healthaxis-backend-search.onrender.com/defaultRows")
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const response = await axios.post("https://healthaxis-backend-search.onrender.com/search", {
        column,
        query,
      })
      setResults(response.data)
